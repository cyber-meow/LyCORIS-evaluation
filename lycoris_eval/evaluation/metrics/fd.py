"""
Taken from https://github.com/layer6ai-labs/dgm-eval/blob/master/dgm_eval/metrics/fd.py
...
def compute_efficient_FD_with_reps(reps1, reps2):
    ...
    return ((mu1 - mu2)**
            2).sum() + sigma1.trace() + sigma2.trace() - 2 * sqrt_trace
...
def compute_FD_infinity(reps1, reps2, num_points=15):
    ...
    fd_batches = np.linspace(min(5000, max(len(reps2) // 10, 2)), len(reps2),
